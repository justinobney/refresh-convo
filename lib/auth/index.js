...
function findOrCreateUser(user, params, context, done){
  context.db.getUsers(params, function(err, res, body, success) {
    ...
    var info = { existingUser: false };

    if (success && body.length) {
      result.user = body[0];
      info.existingUser = true;
      done(err, result, info);
    }
    else {
...
function configurePassport(context)
{
  ...
  app.get('/auth/google/callback', function(req, res, next) {
    passport.authenticate('google', function(err, result, info) {
      return handleAuthCallback(result, info, req, res, next);
    })(req, res, next);
  });
...
function handleAuthCallback(result, info, req, res, next){
  if ( info && !info.existingUser ) { req.session.success = 'New User Created. <a href="/profile">Edit Profile</a>'; }
